<template>
  <div class="home" style="padding: 10px">
      ...
      <el-form inline="true" size="small">
        ...
        <el-form-item style="float: right" v-if="numOfOutDataCar != 0">
          <el-popconfirm confirm-button-text="查看" cancel-button-text="取消" :icon="InfoFilled" icon-color="red"
            title="您有车辆已逾期，请尽快归还" @confirm="toLook">
            <template #reference>
    ...
    <el-table :data="tableData" stripe :border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed="right" label="操作">
        <template v-slot="scope">
          ...
          <el-button type="primary" size="mini" @click="handleLend(scope.row.id, scope.row.id, scope.row.model)"
            v-if="user.role == 2" :disabled="scope.row.status == 0">租借</el-button>
          <el-popconfirm title="确认还车?" @confirm="handleReturn(scope.row.id, scope.row)" v-if="user.role == 2"
            :disabled="scope.row.status == 1">
            <template #reference>
              <el-button type="danger" size="mini" :disabled="this.carIdArray.has(scope.row.carId) ||
                scope.row.status == 1
                ">还车</el-button>
            </template>
          </el-popconfirm>
    ...
    <!-- 逾期详情对话框-->
    <el-dialog v-model="dialogVisible3" v-if="numOfOutDataCar != 0" title="逾期详情" width="50%"
      ...
      <el-dialog v-model="dialogVisible" title="上架车辆" width="30%">
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="生产年份">
            <el-input style="width: 80%" v-model="form.productionYear"></el-input>
          </el-form-item>
        </el-form>
        <template #footer>
          <span class="dialog-footer">
