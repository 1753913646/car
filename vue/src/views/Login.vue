    ...
    login() {
      this.$refs['form'].validate(async (valid) => {
        if (valid) {
          ...
          let res = await request.post("user/login", this.form)
          if (res.status == 200) {
            ElMessage.success("登录成功")
            sessionStorage.setItem("user", JSON.stringify(res.data.user))//缓存用户信息
            console.log(res.data)
            sessionStorage.setItem("token", res.data.token);//缓存token信息

            let res2 = await request.get("user/info")
            console.log("res2 is ", res2)
            if (res2.status == 200) {
              sessionStorage.setItem("user", JSON.stringify(res2.data))//缓存用户信息
