      ...
      <div class="table-container" style="flex: 1; margin-right: 5px;margin-left: 5px;margin-top: 10px;width: 60%;">
        <el-row v-show="showSearch">
          <el-form ref="queryRef" :inline="true" :model="state.queryForm" @keyup.enter="getDataList">
            <el-form-item prop="lotName">
              <el-input v-model="state.queryForm.lotName"  placeholder="请输入路段名称" clearable />
            </el-form-item>
            <el-form-item>
        ...
        <el-row>
          <div class="mb8" style="width: 100%">
            <el-button icon="folder-add" type="primary" class="ml10" @click="this.$refs.formDialogRef.openDialog()"
              v-auth="'carParkingLot_carParkingLot_add'">
              新 增
            </el-button>
            <el-button plain :disabled="multiple" icon="Delete" type="primary"
              v-auth="'carParkingLot_carParkingLot_del'" @click="handleDelete(selectObjs)">
              批量删除
            </el-button>
            <right-toolbar v-model:showSearch="showSearch" :export="'carParkingLot_carParkingLot_export'"
        ...
        <div class="table-container" style="height: 80%;width: 100%">
          <el-table :data="state.dataList" v-loading="state.loading" border :cell-style="tableStyle.cellStyle"
            ...
            @row-click="handleRowClick" height="450px">
            ...
            <el-table-column label="操作" prop="selectPoint" width="180" align="center">
              <template #default="scope">
                <el-button size="small" icon="edit" text type="primary" v-auth="'carParkingLot_carParkingLot_edit'"
                  @click="handleEdit(scope.row.lotId)">
                  {{ text === '' ? '编辑' : '保存' }}
                </el-button>
                <el-button size="small" icon="delete" text type="primary" v-auth="'carParkingLot_carParkingLot_del'"
            ...
            <el-table-column label="选点" prop="selectPoint" width="100" align="center">
              <template #default="scope">
                <el-button size="small" icon="map-location" text type="warning" @click="myAddress(scope.row)">
                  选点
                </el-button>
              </template>
            </el-table-column>
        ...
        <el-pagination v-model:current-page="state.pagination.current" v-model:page-size="state.pagination.size"
          :page-sizes="[10, 20, 50, 100]" :size="state.pagination.size" background="blue"
          layout="sizes, prev, pager, next" :total="1000" @size-change="sizeChangeHandle"
  ...
  methods: {
    getDataList() {
      ...
      request.get('/carLot/page', { params: query }).then(res => {
        this.state.dataList = res.data.list;
