...
<!--      <el-table-column prop="rentalEnd" label="最迟归还日期" />-->
      ...
      <el-table-column fixed="right" label="操作" >
        <template v-slot="scope">
          ...
          <el-popconfirm title="确认续借(续借一次延长10天)?" @confirm="handlereProlong(scope.row)" v-if="user.role == 2" :disabled="scope.row.prolong == 0">
            <template #reference>
    ...
    load(){
      if(this.user.role == 1){
        ...
        request.get("/carwithuser",{
          ...
          this.tableData = res.data.records.filter(record => record.nickName === this.user.nickName);
          this.total = res.data.total
        })
      }
    },
...
      let res=await request.post("/carwithuser/deleteRecord",this.id)
        if(res.status == 200 ){
...
    },
    ...
    //   request.post("carwithuser/deleteRecord",form3).then(res =>{
    //     console.log(res)
    //     if(res.code == 0 ){
    //       ElMessage.success("删除成功")
    //     }
